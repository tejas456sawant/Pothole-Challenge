...
export default class Register extends Component {
  ...
  registerUser() {
    ...
    if (username != '' && number != '') {
      ...
      this.setState({
        errorMessage: 'username or number field is empty',
        loading: false,
      });
    }
  }
    ...
    return (
      <View style={{flex: 1}}>
        ...
        <MaterialButtonViolet1
          ...
          onPress={this.storeData}></MaterialButtonViolet1>
      </View>
    );
  }
  render() {
      ...
      <View style={styles.container}>
        <View style={styles.containGradient}>
          <LinearGradient
            ...
            locations={[0, 0.67]}
            colors={['#ff00c4', '#FF0043']}
            style={styles.linearGradient}></LinearGradient>
        </View>
        <View style={styles.contain1}>
          <MaterialIconTextbox
            placeholderText={'Enter Username'}
            textInputIcon={'account-outline'}
            onChangeText={(text) => this.setState({username: text})}
            style={styles.materialIconTextbox1}></MaterialIconTextbox>
          <MaterialIconTextbox
            placeholderText={'Enter Number'}
            textInputIcon={'phone'}
            style={styles.materialIconTextbox2}
            onChangeText={(text) => this.setState({number: text})}
            keyboardType={'phone-pad'}></MaterialIconTextbox>
          <MaterialIconTextbox
          ...
          <MaterialIconTextbox
            ...
            keyboardType={'email-address'}></MaterialIconTextbox>
          {this.renderButton()}
